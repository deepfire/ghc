  ...
  = do
    let semantic_mod = canonicalizeHomeModule (hsc_dflags hsc_env) (moduleName this_mod)
        ...
        intermediate_iface = ModIface {
              ...
              mi_exports_aliases = mkIfaceExportsAliases exports_aliases,
...
mkIfaceExportsAliases :: [(ModuleName, AvailInfo)] -> [(ModuleName, IfaceExport)]
mkIfaceExportsAliases exports
  = sortBy stableAvailCmpAliases (map (sort_subs <$>) exports)
  where
    sort_subs :: AvailInfo -> AvailInfo
    sort_subs (Avail n) = Avail n
    sort_subs (AvailTC n [] fs) = AvailTC n [] (sort_flds fs)
